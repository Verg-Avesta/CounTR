...
def get_args_parser():
    ...
    parser.add_argument('--output_dir', default='./output_ff11_dir',
                        help='path where to save, empty for no saving')
    parser.add_argument('--log_dir', default='./output_ff11_dir',
...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        sample = {'image':image,'lines_boxes':rects,'gt_density':density, 'dots':dots}
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        for data_iter_step, (samples, gt_density) in enumerate(metric_logger.log_every(data_loader_train, print_freq, header)):
            ...
            for i in range(output.shape[0]):
                pred_cnt = torch.sum(output[i]/100).item()
                gt_cnt = torch.sum(gt_density[i]/100).item()
            ...
            if log_writer is not None and data_iter_step == 0:
                ...
                fig = samples[0][0]/3+samples[0][1]/3+samples[0][2]/3

                log_writer.add_images('heatmap overlay', (fig/2+output[0]/20), int(epoch),dataformats='HW')
                log_writer.add_images('heatmap', (output[0]/10), int(epoch),dataformats='HW')
                log_writer.add_images('gt_density', (fig/2+gt_density[0]/5), int(epoch),dataformats='HW')
            ...
            # Only use 2 batches when overfitting
        ...
        if train_mae/(len(data_loader_train) * args.batch_size) < min_MAE:
            min_MAE = train_mae/(len(data_loader_train) * args.batch_size)
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=6666)
