...
def get_args_parser():
    ...
    parser.add_argument('--resume', default='./output_ff4_dir/checkpoint-1250.pth',
...
class TestData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return sample['image'], sample['dots']
...
def main(args):
    ...
    for data_iter_step, (samples, gt_dots) in enumerate(metric_logger.log_every(data_loader_test, print_freq, header)):
        ...
        if data_iter_step < 3:
            ...
            fig=fig/2+density_map/2
            fig = fig.cpu().numpy()
            plt.matshow(fig)
            plt.savefig(f'./image/pred{data_iter_step}.png')
            plt.show()
        
        pred_cnt = torch.sum(density_map/100).item()
        gt_cnt = gt_dots.shape[1]
        cnt_err = abs(pred_cnt - gt_cnt)
        train_mae += cnt_err
        train_rmse += cnt_err ** 2

        print(f'{data_iter_step}/{len(data_loader_test)}: pred_cnt: {pred_cnt},  gt_cnt: {gt_cnt},  error: {abs(pred_cnt - gt_cnt)},  AE: {cnt_err},  SE: {cnt_err ** 2} ')
    ...
    log_stats = {'Current MAE': train_mae/(len(data_loader_test)),
